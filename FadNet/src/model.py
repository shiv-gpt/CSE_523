...
def build_lat_dis_layers():
    enc_layers = []
    norm_fn = nn.BatchNorm2d
    enc_layers.append(nn.Sequential(nn.Conv2d(3, 32, 4, 2, 1),nn.LeakyReLU(0.2, inplace=True)))
    enc_layers.append(nn.Sequential(nn.Conv2d(32, 64, 4, 2, 1),norm_fn(64, affine=True),nn.LeakyReLU(0.2, inplace=True)))
    enc_layers.append(nn.Sequential(nn.Conv2d(64, 128, 4, 2, 1),norm_fn(128, affine=True),nn.LeakyReLU(0.2, inplace=True)))
    enc_layers.append(nn.Sequential(nn.Conv2d(128, 256, 4, 2, 1),norm_fn(256, affine=True),nn.LeakyReLU(0.2, inplace=True)))
    enc_layers.append(nn.Sequential(nn.Conv2d(256, 512, 4, 2, 1),norm_fn(512, affine=True),nn.LeakyReLU(0.2, inplace=True)))
    enc_layers.append(nn.Sequential(nn.Conv2d(512, 512, 4, 2, 1),norm_fn(512, affine=True),nn.LeakyReLU(0.2, inplace=True)))
    return enc_layers
...
def build_ae_layers():
    ...
    # enc_layers.append(nn.Sequential())
    enc_layers.append(nn.Sequential(nn.Conv2d(128, 256, 4, 2, 1),norm_fn(256, affine=True),nn.LeakyReLU(0.2, inplace=True)))
    enc_layers.append(nn.Sequential(nn.Conv2d(256, 512, 4, 2, 1),norm_fn(512, affine=True)))
    # enc_layers.append(nn.Sequential())

    # dec_layers.append(nn.Sequential(nn.ConvTranspose2d(514, 512, 4, 2, 1, bias=False),norm_fn(512, affine=True),nn.ReLU(inplace=True)))
    # dec_layers.append(nn.Sequential())
    # dec_layers.append(nn.Sequential())
    dec_layers.append(nn.Sequential(nn.ConvTranspose2d(514, 256, 4, 2, 1, bias=False),norm_fn(256, affine=True),nn.ReLU(inplace=True)))
    dec_layers.append(nn.Sequential(nn.ConvTranspose2d(258, 128, 4, 2, 1, bias=False),norm_fn(128, affine=True),nn.ReLU(inplace=True)))
    dec_layers.append(nn.Sequential(nn.ConvTranspose2d(130, 64, 4, 2, 1, bias=False),norm_fn(64, affine=True),nn.ReLU(inplace=True)))
    dec_layers.append(nn.Sequential(nn.ConvTranspose2d(66, 32, 4, 2, 1, bias=False),norm_fn(32, affine=True),nn.ReLU(inplace=True)))
    dec_layers.append(nn.Sequential(nn.ConvTranspose2d(34, 3, 4, 2, 1, bias=False),nn.Tanh()))
...
def build_layers(img_sz, img_fm, init_fm, max_fm, n_layers, n_attr, n_skip,
	...
	print("n_in == %d" %n_in)
        print("n_out == %d" %n_out)
	print("n_dec_in == %d" %n_dec_in)
        print("n_dec_out == %d" %n_dec_out)
...
def build_layers(img_sz, img_fm, init_fm, max_fm, n_layers, n_attr, n_skip,
	...
	#print("n_dec_out == %d" %n_dec_out)
   	enc_layer.append(nn.Conv2d(n_in, n_out, 4, 2, 1))
        ...
        dec_layer.append(nn.ConvTranspose2d(n_dec_in, n_dec_out, 4, 2, 1, bias=False))
        if(i==(n_layers-1)):
        	dec_layer.append(nn.Hardtanh(0,1))
        elif i > 0:
        	dec_layer.append(norm_fn(n_dec_out, affine=True))
        	dec_layer.append(nn.ReLU(inplace=True))
        else:
...
class AutoEncoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        enc_layers, dec_layers = build_ae_layers()
...
class LatentDiscriminator(nn.Module):
	...
	enc_layers = build_lat_dis_layers() 
